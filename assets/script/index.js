...
function createGameScene(){
    ...
    let timer=window.setInterval(() => {
        ...
        if (count >0) {
            ...
            scoreObj.date = new Date().toLocaleDateString();
            scoreObj.hits = score;
            scoreObj.percentage = (score / arrWords.length).toFixed(2) * 100;
...
function createEndScene(){
    ...
    const pTitle = document.createElement("p");
    pTitle.classList.add("home-title");
    pTitle.innerHTML = `Score: ${scoreObj.hits}`;
    divHome.appendChild(pTitle);
    //start button
    const btnRetart = document.createElement("button");
